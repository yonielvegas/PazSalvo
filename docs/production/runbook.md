...
Para el bootstrap inicial sin migraciones, dejar `PRODUCTION_HEALTH_URL` sin configurar hasta que `current` esté activo y se hayan aplicado manualmente las migraciones aprobadas desde esa release: `cd /var/www/paz-salvo-private/current` y `/usr/bin/php8.4 -d memory_limit=256M artisan migrate --force`. Después comprobar `/healthz` y configurar la variable del Environment. Si se configura antes, `/healthz` puede devolver 503 por la tabla `cache` ausente y el CD revertirá la activación del primer deploy.
