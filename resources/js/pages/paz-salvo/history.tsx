...
type Doc = { id:number; folio:string; client_number:string; holder_name:string; agency_name_snapshot:string; generated_by_name_snapshot:string; issued_at:string; expires_at:string; status:string; effective_status:string };
...
export default function History({ documents, filters, agencies, users }: { documents:Page<Doc>; filters:Record<string,string>; agencies:{id:number;name:string}[]; users:{id:number;name:string}[] }) {
    ...
    return <AppLayout><Head title="Historial" /><section className="page-heading"><p className="eyebrow">DOCUMENTOS OFICIALES</p><h1>Historial de certificados</h1><p>{documents.total} registros encontrados</p></section>
        ...
        <div className="table-card"><table><thead><tr><th>Folio</th><th>NAC / Cliente</th><th>Agencia</th><th>Elaborado por</th><th>Emisión</th><th>Estado</th><th /></tr></thead><tbody>{documents.data.map(d=><tr key={d.id}><td><b>{d.folio}</b></td><td>{d.client_number}<small>{d.holder_name}</small></td><td>{d.agency_name_snapshot}</td><td>{d.generated_by_name_snapshot}</td><td>{new Date(d.issued_at).toLocaleString('es-PA')}</td><td><span className={`status ${d.effective_status}`}>{d.effective_status}</span></td><td><Link className="icon-link" href={`/paz-salvos/${d.id}`}><Eye /></Link></td></tr>)}</tbody></table>{!documents.data.length&&<p className="empty">No hay certificados para estos filtros.</p>}</div>
