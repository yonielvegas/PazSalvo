...
export function DebtModal({ result, onClose }: { result: QueryResult; onClose: () => void }) {
    ...
    const debts = result.debts.filter((debt) => debt.amount > 0);
    return (
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
            <section className="debt-modal" role="dialog" aria-modal="true" aria-labelledby="debt-title">
                ...
                <p>El cliente mantiene saldo pendiente.</p>
                <strong className="debt-total">{money(result.balances.total_balance)}</strong>
                {debts.length > 0 && <div className="debt-list">{debts.map((debt, index) => <div key={index}><span>{debt.document_type || debt.period || 'Saldo pendiente'}</span><b>{money(debt.amount)}</b></div>)}</div>}
